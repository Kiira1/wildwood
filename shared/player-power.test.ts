...
describe("player power", () => {
  it("uses the same research and equipped-item stats everywhere", () => {
    ...
    expect(effective.damage).toBeCloseTo(130);
  ...
  it("boosts gear health with Vitality without multiplying already-researched saved health twice", () => {
    ...
    expect(effectivePlayerPowerStats(stats, { vitality: 10 }).maxHp).toBeCloseTo(270);
  ...
  it("adds chest health and helmet regeneration separately", () => {
    ...
    expect(effective.maxHp).toBeCloseTo(7300);
  });
