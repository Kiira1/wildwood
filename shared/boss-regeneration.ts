...
export function bossHitsToDefeat(maxHp: number, hitDamage: number, attackInterval: number) {
  if (hitDamage >= maxHp) return 1;
  const netDamage = hitDamage - maxHp * BOSS_REGEN_FRACTION_PER_SECOND * attackInterval;
