...
/** Fixed map-entry bonuses. +10 adds 80%, so even maxed gear supplies at most
...
export function equipmentStatDefinition(tier: number, slot: "HAND" | "HEAD" | "CHEST", quality = 1) {
  ...
  if (slot === "HAND") return { stats: [`DAMAGE +${amount}`], weapon: { mode: "RANGED" as const, projectile: "ARROW" as const, damageBonus: amount } };
