...
try {
  const players = [];
  // One past capacity, so the run both fills an instance and crosses into the
  // next one. Filling it also exercises synchronisation at full occupancy,
  // which is where a limit left behind at the old capacity would surface.
  const accounts = MAP_SHARD_CAPACITY + 1;
  ...
  assert.deepEqual([...occupancy.values()].sort((a, b) => b - a), [MAP_SHARD_CAPACITY, 1]);
  ...
  assert.equal(regions[0].db.player.count(), BigInt(MAP_SHARD_CAPACITY));
  assert.equal(regions[accounts - 1].db.player.count(), 1n);
  ...
  const otherHp = [...regions[accounts - 1].db.dragonBoss.iter()][0].hp;
  ...
  assert.equal([...regions[accounts - 1].db.dragonBoss.iter()][0].hp, otherHp);
  ...
  console.log(`PASS: ${accounts} accounts route to separate ${MAP_SHARD_CAPACITY}/1 databases; movement stays regional; account saves are unavailable in map databases.`);
} finally {
