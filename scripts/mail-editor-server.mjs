...
const copyPath = resolve(root, 'shared/mailbox-copy.json');
const htmlPath = resolve(root, 'tools/mail-editor/index.html');
const port = 4176;
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    for (const key of Object.keys(current)) {
      if (typeof input?.[key] !== 'string' || !input[key].trim() || input[key].length > (key === 'title' ? 100 : 6000))
        return respond(400, { error: 'Use a title of up to 100 characters and a message of up to 6,000.' });
...
server.listen(port, '127.0.0.1', () => console.log(`Mail text editor: http://127.0.0.1:${port}\nSaves to shared/mailbox-copy.json`));
