...
export function buildGuildReplayTimeline(battle: GuildBattleResult) {
  ...
  for (let step = 1; step < frames.length; step++) {
    ...
    for (const [target, amount] of hits) if (amount > 0) {
      damage.push({ target, amount, time: frame.time, x: frame.actors[target].x, y: frame.actors[target].y });
    }
  }
  const shots = attacks.flat().sort((a, b) => a.launch - b.launch);
  return { fighters, frames, deaths, attacks, shots, damage, sample };
}
