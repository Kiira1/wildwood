...
describe("vendor weapon baselines", () => {
  it("uses the actual game tuning for matching artwork, and natural dimensions for source defaults", () => {
    const weapon = { id: "vendor:bow", label: "Bow", category: "Bow", source: "/art-source/vendor/bow.webp", gameItemId: "starter_bow" };
  ...
  it("applies the shared sword category only in game mode", () => {
    const weapon = { id: "vendor:sword", label: "Sword", category: "Sword", source: "/art-source/vendor/sword.webp" };
  ...
  it("resets tuned armor to natural artwork at the shared chest anchor", () => {
    expect(sourceDefaultPresentation({ kind: "SPRITE", layer: "CHEST", source: "armor.webp", width: 76, height: 68, top: 100 }))
      .toEqual({ kind: "SPRITE", layer: "CHEST", source: "armor.webp", bottom: 168 });
  });
});
